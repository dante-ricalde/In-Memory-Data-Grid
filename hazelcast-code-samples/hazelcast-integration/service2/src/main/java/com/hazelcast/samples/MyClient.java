...
public class MyClient implements CommandLineRunner {
	...
	public void run(String... arg0) throws Exception {
		...
		// center console
		Config config = new XmlConfigBuilder("/configFolder/member/hz.service.embedded.xml").build();
				//.setProperties(hzService1EmbeddedProperties).build();
		String hostIp = InetAddress.getLocalHost().getHostAddress();
		config.getGroupConfig().setName(hostIp).setPassword(hostIp);
		HazelcastInstance newHazelcastInstance = Hazelcast.newHazelcastInstance(config);
		logger.debug("HZ EMBEDDED INSTANCE STARTED ...");
		logger.debug("STARTING HZ EMBEDDED INSTANCE CLIENT");
		XmlClientConfigBuilder xmlClientConfigBuilder = new XmlClientConfigBuilder("/configFolder/member/hz.service.embedded-client.xml");
//		xmlClientConfigBuilder.setProperties(hzService1EmbeddedProperties);
		ClientConfig xmlEmbeddedClientConfig = xmlClientConfigBuilder.build();
		xmlEmbeddedClientConfig.getGroupConfig().setName(hostIp).setPassword(hostIp);
		final HazelcastInstance embeddedClient = HazelcastClient.newHazelcastClient(xmlEmbeddedClientConfig);
